...
export default function PlatformPage({user}){
 ...
 const [aiDraft,setAiDraft]=useState(null),[projectForm,setProjectForm]=useState(emptyProject),[note,setNote]=useState({body:'',source_url:'',document_id:''});
 ...
 const [adminSubmissions,setAdminSubmissions]=useState([]),[selectedChallenge,setSelectedChallenge]=useState(''),[graph,setGraph]=useState({nodes:[],edges:[]}),[docs,setDocs]=useState([]);
 const [coverageStates,setCoverageStates]=useState('Uttar Pradesh,Delhi'),[coverageTopics,setCoverageTopics]=useState('urban expansion,flood exposure'),[coverage,setCoverage]=useState(null),[chosenDoc,setChosenDoc]=useState(''),[recommendations,setRecommendations]=useState([]);
 ...
 return <div className="hub"><header className="hub-header"><div className="eyebrow">STAGES 3–5 · INTEGRATED PROTOTYPE</div><h2>Research & Policy Hub</h2><p>Collaborative projects, sourced indicators, transparent scenarios and evidence discovery.</p></header>
  ...
  {tab==='Research Workspaces'&&<div className="hub-grid">
   ...
   {selection&&<Panel title={'Evidence notebook: '+selection.title}>
    ...
    <button type="button" onClick={exportReport}>Export evidence report (.md)</button><button type="button" onClick={async()=>{try{setAiDraft(await request("/platform/projects/"+chosen+"/ai-brief",{method:"POST"}))}catch(e){setError(e.message)}}}>Draft AI evidence synthesis</button>{aiDraft&&<div className="scenario-result"><strong>LLM evidence synthesis (review before reuse)</strong><p className="ai-draft">{aiDraft.draft}</p><small>{aiDraft.notice}</small></div>}
  ...
  {tab==='Evidence Graph'&&<div className="hub-grid">
   ...
   <Panel title="Research coverage and discovery"><p>Identify regions that have limited coverage in <strong>our indexed documents</strong>, not in the entire research literature.</p>{insights?.by_region?.map((r,i)=><p key={i}>{r.state} / {r.district}: <strong>{r.count} indexed documents</strong></p>)}
    <h4>Compare indexed semantic coverage by state and topic</h4><form onSubmit={async e=>{e.preventDefault();try{setCoverage(await request('/platform/research-coverage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({states:coverageStates.split(',').map(x=>x.trim()).filter(Boolean),topics:coverageTopics.split(',').map(x=>x.trim()).filter(Boolean)})}))}catch(e){setError(e.message)}}}><label>States (comma separated)<input value={coverageStates} onChange={e=>setCoverageStates(e.target.value)}/></label><label>Research topics (comma separated)<input value={coverageTopics} onChange={e=>setCoverageTopics(e.target.value)}/></label><button>Analyze repository coverage</button></form>{coverage&&<div><p className="hub-caution">{coverage.disclaimer}</p><table><thead><tr><th>State</th><th>Topic</th><th>Indexed matches</th></tr></thead><tbody>{coverage.results.map((r,i)=><tr key={i}><td>{r.state}</td><td>{r.topic}</td><td>{r.matching_indexed_documents}</td></tr>)}</tbody></table></div>}
