...
function App({user}){
 ...
 async function reload(filters=filter){try{const params=new URLSearchParams(Object.entries(filters).filter(([k,v])=>v.trim()));const r=await apiFetch(API+'/documents?'+params.toString());if(!r.ok)throw Error('Backend unavailable');setDocs(await r.json())}catch(e){setNotice('Start PostgreSQL and FastAPI: '+e.message)}}
 useEffect(()=>{reload()},[]);
 async function upload(e){
  ...
  try{
   ...
   for(let attempt=0;attempt<600;attempt++){
    const response=await apiFetch(API+'/documents/jobs/'+job.job_id,{cache:'no-store'});
 ...
 async function removeDoc(doc){if(!window.confirm('Delete '+(doc.title||doc.filename)+' and its search index?'))return;try{const r=await apiFetch(API+'/documents/'+doc.id,{method:'DELETE'});if(!r.ok)throw Error('Delete failed');await reload();setNotice('Document deleted')}catch(e){setNotice(e.message)}}
 async function ask(e){e.preventDefault();if(!question.trim())return;setBusy(true);setAnswer(null);setNotice('');try{const r=await apiFetch(API+'/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question,state:filter.state,district:filter.district})});const d=await r.json();if(!r.ok)throw Error(d.detail||'Request failed');setAnswer(d)}catch(e){setNotice(e.message)}finally{setBusy(false)}}
